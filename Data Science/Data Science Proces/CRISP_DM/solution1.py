...
def higher_ed(x): return 1 if x in ("Master's degree",
                                    "Doctoral", "Professional degree") else 0
    ...
    for col in cat_cols:
        try:
            ...
            df = pd.concat([df.drop(col, axis=1), pd.get_dummies(
                df[col], prefix=col, prefix_sep='_', drop_first=True, dummy_na=dummy_na)], axis=1)
        except:
            continue
    return df
